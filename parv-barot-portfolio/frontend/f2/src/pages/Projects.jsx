  ...
  return (
    <div style={{ paddingTop: 'var(--nav-h)' }}>
      ...
      <section style={{ padding: '0 0 var(--section-gap)' }}>
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 1, background: 'var(--border2)' }}>
            {visible.map(p => {
              return (
                <Link key={p.id} to={`/projects/${p.id}`} className="hover-card" style={{ display: 'block', textDecoration: 'none' }}>
                  <div style={{
                    ...
                    border: '1px solid var(--border2)', borderRadius: 0, cursor: 'pointer',
                  }}
                  >
                    ...
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 20 }}>
                      {p.tracks.map(t => (
                        <span key={t} style={{ fontSize: 10, padding: '3px 8px', background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 0, color: 'var(--text3)', fontFamily: 'var(--font-body)', letterSpacing: '0.05em', textTransform: 'uppercase' }}>{t}</span>
                      ))}
                    </div>
                    ...
                    <div style={{ paddingTop: 16, borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <span style={{ fontFamily: 'var(--font-display)', fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--accent)' }}>View Case Study →</span>
                    </div>
                  </div>
                </Link>
              );
