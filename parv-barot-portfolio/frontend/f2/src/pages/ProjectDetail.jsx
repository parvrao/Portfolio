    ...
    <div style={{ paddingTop: 'var(--nav-h)' }}>
      ...
      <section style={{ padding: '80px 0 64px', borderBottom: '1px solid var(--border)', background: 'var(--bg2)' }}>
        <div className="container">
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 24, marginBottom: 40 }}>
            ...
            {/* Sidebar card */}
            <div style={{ background: 'var(--bg3)', border: '1px solid var(--border2)', borderRadius: 0, padding: 24, minWidth: 220 }}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {p.tracks.map(t => (
                    <span key={t} style={{ fontSize: 10, padding: '3px 8px', border: '1px solid var(--border2)', borderRadius: 0, color: 'var(--text3)', fontFamily: 'var(--font-display)', letterSpacing: '0.05em' }}>{t}</span>
                  ))}
                </div>
              </div>
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {p.tools.map(t => (
                    <span key={t} style={{ fontSize: 10, padding: '3px 8px', background: 'var(--accent-dim)', border: '1px solid var(--border)', borderRadius: 0, color: 'var(--accent)', fontFamily: 'var(--font-display)', letterSpacing: '0.05em' }}>{t}</span>
                  ))}
                </div>
              </div>
              <div>
                ...
                <a href={p.isLive ? p.liveLink : p.datasetLink} target="_blank" rel="noreferrer"
                  style={{ fontSize: 12, color: 'var(--accent)', textDecoration: 'underline', textDecorationColor: 'var(--border2)', lineHeight: 1.4, display: 'block' }}>
          ...
          {/* Key metric */}
          <div style={{
            background: 'var(--accent-dim)', border: '1px solid var(--border)', borderRadius: 0,
            padding: '24px 32px', display: 'inline-flex', alignItems: 'center', gap: 24
          }}>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: 52, fontWeight: 800, color: 'var(--accent)', lineHeight: 1 }}>{p.heroStat}</div>
            <div>
      ...
      <div className="container" style={{ padding: '80px 40px' }}>
        <div style={{ maxWidth: 800 }}>
          ...
          {/* Problem */}
          <div style={{ marginBottom: 64, background: 'var(--bg2)', border: '1px solid var(--border2)', borderLeft: '3px solid var(--accent)', padding: 32, borderRadius: 0 }}>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: 12, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--accent)', marginBottom: 12 }}>The Problem</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              {p.methodology.map(m => (
                <div key={m.step} style={{ display: 'flex', gap: 24, alignItems: 'flex-start' }}>
                  <div style={{
                    flexShrink: 0, width: 40, height: 40, borderRadius: 0,
                    background: 'var(--accent-dim)', border: '1px solid var(--border)',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontFamily: 'var(--font-display)', fontSize: 11, fontWeight: 800, color: 'var(--accent)', letterSpacing: '0.05em'
                  }}>{m.step}</div>
                  <div>
          ...
          <div style={{ marginBottom: 64 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 16 }}>
              {p.results.map(r => (
                <div key={r.metric} style={{ background: 'var(--bg2)', border: '1px solid var(--border2)', borderRadius: 0, padding: '20px 16px' }}>
                  <div style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(14px,2vw,22px)', fontWeight: 800, color: 'var(--accent)', lineHeight: 1.2, marginBottom: 4, wordBreak: 'break-word' }}>{r.value}</div>
          ...
          {/* Key learning */}
          <div style={{ marginBottom: 64, background: 'var(--accent-dim)', border: '1px solid var(--border)', borderRadius: 0, padding: 32 }}>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: 12, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--accent)', marginBottom: 12 }}>Key Learning</div>
        ...
        <div style={{ borderTop: '1px solid var(--border2)', paddingTop: 48, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
          {prev ? (
            <Link to={`/projects/${prev.id}`} style={{
              ...
              borderRadius: 0, display: 'block', transition: 'background 0.2s', textDecoration: 'none',
            }}
          ...
          {next ? (
            <Link to={`/projects/${next.id}`} style={{
              padding: 24, background: 'var(--bg2)', border: '1px solid var(--border2)',
              borderRadius: 0, display: 'block', textAlign: 'right', transition: 'background 0.2s', textDecoration: 'none',
            }}
              onMouseEnter={e => e.currentTarget.style.background = 'var(--surface)'}
              onMouseLeave={e => e.currentTarget.style.background = 'var(--bg2)'}
            >
